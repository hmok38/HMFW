...
namespace HMFW.SampleURP
{
    ...
    public class TestClass
    {
        public void Test()
        {
             ...
             * 业务层一定要通过FW.的方式访问所有接口,甚至第三方插件也可以这样来访问
            */
            ...
            Debug.Log(FW.SampleMgr.SampleAMethod()); //结果:1;--访问的还是SampleMgrBase的接口
            FW.SampleMgr = new SampleMgrNew();
            Debug.Log(FW.SampleMgr.SampleAMethod()); //结果:2;--访问的是 SampleMgrNew 的接口了
            //新模块的新函数无法通过api接口访问到,除非是用扩展的方式做的
            //FW.SampleMgr.SampleMethodB();
            //对新函数再次扩展后就可以访问了
            FW.SampleMgr.SampleMethodC();
             ...
             */
            //---业务中访问新扩展的功能模块的方法
            FW.CustomAPI.FWTestMgr().FWTestMgrMethod();
        }
    }
    ...
    public class SampleMgrNew : SampleMgrBase
    {
        ...
        /// <summary>
        /// 新模块的新函数是不能通过 FW.SampleMgr.SampleBMethod() 访问,必须通过扩展的方式创建的函数才可以访问
        /// </summary>
        public void SampleMethodB() //无法 FW.SampleMgr.SampleMethodB 访问
        {
        }

        /// <summary>
        /// 新模块未扩展新函数是不能通过 FW.SampleMgr.SampleCMethod() 访问,必须通过扩展的方式创建的函数才可以访问
        /// 可以再对基类扩展一次它 请参考:SampleMgrNewExtend类
        /// </summary>
        public void SampleMethodC() //未扩展的话无法 FW.SampleMgr.SampleMethodC 访问
        {
        }
    }
